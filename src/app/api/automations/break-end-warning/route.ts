    ...
    try {
      ...
      for (const entry of activeBreaks) {
        ...
        try {
          ...
          log.error(
            `[break-end-warning] push failed for user ${user.id} entry ${entry.id}`,
            {
              error:
          ...
          captureRouteError(pushErr, {
            route: "/api/automations/break-end-warning",
            method: "POST",
          });
        }
      ...
      log.error("[break-end-warning] cron fatal error", { error: err });
