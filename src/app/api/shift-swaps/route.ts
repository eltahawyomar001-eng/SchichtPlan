...
export const POST = withRoute("/api/shift-swaps", "POST", async (req) => {
  const auth = await requireAuth();
  if (!auth.ok) return auth.response;
  const { user, workspaceId } = auth;
  ...
  if (existingSwap) {
    return NextResponse.json(
      {
        error: "Für diese Schicht liegt bereits ein offener Tauschantrag vor.",
      },
      { status: 409 },
    );
  }
  ...
  if (body.targetId && body.targetShiftId) {
    ...
    if (blockingConflicts.length > 0) {
      return NextResponse.json(
        {
          error: "Tausch nicht möglich — ArbZG-Verstoß für den Tauschpartner.",
    ...
    if (targetShift) {
      const requesterConflicts = await checkShiftConflicts({
        ...
        date: new Date(targetShift.date).toLocaleDateString("en-CA", {
          timeZone: "Europe/Berlin",
        }),
