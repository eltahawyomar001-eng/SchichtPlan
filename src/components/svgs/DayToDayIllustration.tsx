 ...
 *
 * Shows a live dashboard with active shifts, clock-in status,
 * and team activity feed for daily operations management.
...
export function DayToDayIllustration() {
  ...
  return (
    ...
    >
      ...
      >
        {/* Active shifts panel */}
        <div className="absolute left-0 top-0 w-[300px] h-full rounded-2xl bg-white shadow-[0px_4px_24px_0px_rgba(124,58,237,0.08),0px_1px_3px_0px_rgba(0,0,0,0.06)] border border-gray-100 overflow-hidden">
          {/* Header */}
          ...
          {/* Summary bar */}
          <div className="mx-4 px-3 py-2 rounded-lg bg-violet-50 flex items-center justify-between">
            <span className="text-xs text-violet-700 font-medium">
              2/3 eingecheckt
            </span>
            <div className="w-20 h-1.5 rounded-full bg-violet-200 overflow-hidden">
              <div className="w-[66%] h-full rounded-full bg-violet-500" />
            </div>
          </div>
        </div>
        ...
        <svg
          ...
          aria-hidden="true"
        >
          <circle cx="5" cy="40" r="3" fill="#7C3AED" fillOpacity="0.3" />
          <circle cx="15" cy="120" r="3" fill="#7C3AED" fillOpacity="0.2" />
          <circle cx="10" cy="200" r="3" fill="#7C3AED" fillOpacity="0.15" />
          <path
